...
function takePhoto() {
    ...
    canvas.toBlob((blob) => {
        ...
        fetch(uploadRoute, {
            // Change the endpoint accordingly
            method: "POST",
            body: formData,
        })
            ...
            .then((data) => {
                ...
                fetch(recentPhotosRoute)
                    ...
                    .then((data) => {
                        ...
                        images = Array.from(galleryImagesOld).map(
                            (img) => img.src
                        );
                        ...
                        const gallery =
                            document.getElementsByClassName("img-gallery")[0];
                        ...
                        for (const image of data) {
                            ...
                            const iconItemDownload =
                                document.createElement("div");
                            iconItemDownload.classList.add(
                                "icon-item",
                                "download"
                            );
                            iconItemDownload.onclick = () =>
                                downloadImage(image);
                        ...
                        const galleryImages =
                            document.querySelectorAll(".gallery-image");
                        images = Array.from(galleryImages).map(
                            (img) => img.src
                        );
