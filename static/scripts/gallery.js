...
function bulkDownload() {
    const selectedImages = Array.from(
        document.querySelectorAll(".image-select:checked")
    ...
    selectedImages.forEach((image, index) => {
        setTimeout(() => {
            downloadImage(
                image,
                `media_${index + 1}${image.includes(".mp4") ? ".mp4" : ".jpg"}`
            );
        }, index * 500);
    });
}
...
function bulkDelete() {
    ...
    if (
        !confirm(
            `Are you sure you want to delete ${selectedImages.length} item(s)? THIS ACTION IS IRREVERSIBLE!`
        )
    ) {
        return;
    }
